...
While the platforms mentioned above run code on their servers, another lightweight category runs Python directly inside your browser. For example, you can try these in-browser environments built on [Pyodide](https://qizhen.xyz/pyodide) or [Brython](https://qizhen.xyz/brython). These are incredibly convenient for quick experiments. They work differently under the hood: Pyodide compiles the CPython interpreter itself to WebAssembly (not JavaScript), so real Python runs in the browser; Brython, by contrast, translates Python code into JavaScript before executing it.
