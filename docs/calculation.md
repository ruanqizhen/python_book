...
Pythora 星球的人们经常把 Python 当作计算器来用，尤其是需要处理大量数据时，它比普通计算器还要方便。Python 不仅支持基本的数学运算，如加法、减法、乘法和除法，还支持更复杂的数学操作，如指数运算、模运算、整数除法等。
...
Python 和大多数语言一样，使用星号 `*` 表示乘法，使用斜杠 `/` 表示除法。在 Python 2 中，整数 the `/` 运算返回的依然是整数结果；但在目前主流的 Python 3 中，整数的 `/` 运算结果会是浮点数（例如 `8 / 2` 也会返回 `4.0`）。Python 3 使用双斜杠 `//` 表示整除运算；使用百分号 `%` 表示取模（余数）运算；使用双星号 `**` 表示幂运算：
...
`+` 和 `-` 除了表示加法减法之外，还被用来表示正负数，比如 `+2`、`-5` 等，正负号的运算优先级要高于其他算术运算符。那么大家猜一下运行下面的程序会出现什么结果？
...
使用 Python 的内置函数，可以将一个整数转换成不同进制的字符串：

* `bin(x)`：将整数 x 转换为二进制字符串。
* `oct(x)`：将整数 x 转换为八进制字符串。
* `hex(x)`：将整数 x 转换为十六进制字符串。
* `int(string, base)`：将字符串转换为整数。如果字符串带有 0b 或 0x 等前缀，可以将 base 设为 0，Python 会自动识别进制。
...
y = '0b101010'
print(int(y, 0))  # 输出 42，基数写 0 表示根据前缀自动识别进制
```

使用函数 `int()` 可以把其他类型的数据转换成整数。
...
### 表示法
浮点数可能非常大或包含许多小数位，为了便于阅读，程序中可能会使用不同的表示方法。最常规的表示法是直接使用小数点。例如：3.14, 0.001, 4.0 等。
...
-inf + 任何负数 = -inf
-inf - 任何负数 = -inf
inf + -inf = nan（正无穷加负无穷是未定义的，所以结果是 nan）
...
由于这种舍入误差，对浮点数进行相等性测试时需要特别小心。通常建议使用一个极小的容差值，而不是直接比较两个浮点数是否完全相等。

## 更复杂的数学运算

除了基本的算术运算外，Python 还提供了许多内置函数来支持数值计算，如 `abs()`、`round()`、`max()`、`min()`、`sum()` 等。这些函数非常直观，且命名与常规数学运算保持一致，很容易望文生义。例如：

```python
print(abs(-5))      # 输出 5，表示绝对值计算
print(round(2.6))   # 输出 3
print(round(2.5))   # 输出 2，注意：Python 3 采用“银行家舍入”，.5 会舍入到最近的偶数
print(round(3.5))   # 输出 4
```

Python 还支持复数运算和一些更高级的数学函数，使其成为处理数学问题的强大工具。在科学计算、数据分析等领域，Python 尤其受欢迎，部分原因是因为它的这种灵活性和强大的数学运算能力。Python 的高级数学函数大多包含在内置的 `math` 和 `statistics` 标准库中，后文会对它们进行详细介绍。这里先看一个简单的示例，例如计算 `2.3` 的平方根，可以调用 `math.sqrt()` 函数：
...
```python
print((3 + 4j) + (2 - 3j))  # 输出 (5+1j)
print((3 + 4j) - (2 - 3j))  # 输出 (1+7j)
print((3 + 4j) * (2 - 3j))  # 输出 (18-1j)
print((3 + 4j) / (2 - 3j))  # 输出 (-0.46153846153846156+1.3076923076923077j)
```

Python 自带的 math 标准库中的函数是用于实数运算的，不能用于复数运算。复数运算可以调用内置的 `cmath` 库中的相应函数。例如，计算 `2 + 3j` 的平方根：
...
布尔值只有两种可能，即 True 和 False。它们实际上是整数的子类型，其中 `True` 等价于 `1`，`False` 等价于 `0`。

### 数据比较

在 Python 中，比较数值大小，是否相等的操作，会返回一个布尔值结果。以下是进行数值大小比较的常用运算符：

大于号 `>` 检查左边的值是否大于右边的值；小于号 `<` 检查左边的值是否小于右边的值；大于或等于号 `>=` 检查左边的值是否大于或等于右边的值；小于或等于号 `<=` 检查左边的值是否小于或等于右边的值；等于号 `==` 检查两个值是否相等；不等于号 `!=` 检查两个值是否不相等。下面是一些示例：
...
例如，在比较字符串时，`"apple" < "banana"` 的结果是 `True`。 需要注意的是，字符串比较是基于字符编码（Unicode/ASCII）的，大写字母的编码总是小于小写字母。因此，`"apple" < "Banana"` 的结果是 `False`（因为 'a' 的编码大于 'B'）。
...
有时候，我们可以使用一种非常直观的方式来进行多个条件的比较，也就是链式比较。比如，我们需要判断 `x > 1` 并且 `x < 2` ，那么可以直接写成 `1 < x < 2`。这种链式比较的工作原理是：Python 会按照从左到右的顺序依次进行每个比较操作。比如对于 `1 < x < 2`，会首先检查 `1 < x` 是否为真。如果为真，接着检查 `x < 2`。只有当这两个条件都为真时，整个表达式才会返回真。如果任何一个条件失败，则整个表达式立即返回假。
...
虽然其他一些运算符也可以用在链式比较中，但那样通常会降低程序的可读性，因此不推荐使用。
...
在上面的示例中，`abs()` 是一个内置函数，它返回输入数据的绝对值。比较的逻辑是：如果 x 和 y 之间的差值小于 epsilon，则认为它们是"相等"的。选择合适的 epsilon 值是很重要的。对于大多数应用场景，1e-9 或 1e-12 通常足够了。

实际上，Python 的 math 库已经为我们内置了一个用于处理浮点数相等判断的最佳工具：
...
nan 与任何值（包括其自身）用 ==、<、> 等比较都会返回 False，但用 != 比较会返回 True（即 nan != nan 的结果是 True）。
...
上文提到的比较都是针对数据的比较，比如 a = 5; b = 5 那么就一定会有 a == b 为 True。但是，前文介绍过，[不同的变量是可以指向同一个数据的](variable#变量的变)。那么我们该如何判断，两个变量究竟是指向了同一个数据，还是指向了值相等的不同数据呢？
...
print(not_a_number == not_a_number)       # 输出 False，这是 IEEE 754 标准规定的
print(not_a_number is not_a_number)       # 输出 True

# 下面这一行输出 True，但这仅仅是因为列表在比较时做了一个优化：
# 如果两个元素指向内存中同一个对象（id 相同），就直接认为相等，不再进行数值比较。
print([not_a_number] == [not_a_number])   # 输出 True
```

在 Python 中，有些数据是确定只有全局一份的，比如 None 这个数据，它是 NoneType 类型的唯一数据，通常用于初始化变量、表示默认状态、进行空值检查等场景。这个数据在全局就只有一份，任何值为 None 的变量，都一定是指向了这个数据。因此，任何值为 `None` 的变量都必然指向同一个数据。在 Python 中，判断一个变量是否为 `None` 时，通常不使用 `==` 操作符，而是使用 `is` 操作符。
...
对于可变数据类型的数据，Python 通常只能为两份数据分配不同的内存空间，以防止其中一个数据的变化影响到其他数据。有时候，当前一个数据可以被移除内存时，Python 也会重用那块内存，可能会造成两个可变数据类型的数据有相同的内存空间。
...
在使用逻辑运算时，`False`、`None`、`0`、空集合、空序列等会被视为假（`False`）；其他所有值都被视为真（`True`）。比如：
...
and 和 or 运算稍微复杂一点：
* 做 `and` 运算时，如果第一个数据为假则返回第一个数据，否则返回第二个数据；
* 做 `or` 运算时，如果第一个数据为真则返回第一个数据，否则返回第二个数据；
...
复合赋值运算符是 Python 中的一种简化赋值操作的方式，它将赋值操作和其他操作（如加法、减法、乘法等）结合在一起。复合赋值运算符通常用于简化代码和提高可读性。比如加法赋值运算符是 `+=`， 它表示把运算符两边的数据或变量相加，再把结果赋值给运算符左边的变量。
...
print(a)  # 输出 7
```
Python 中的常用复合赋值运算符包括： `+=` : 加法赋值； `-=` : 减法赋值； `*=` : 乘法赋值； `/=` : 除法赋值； `%=` : 取模赋值（返回除法的余数）； `//=` : 整除赋值（返回除法的商，忽略余数）； `**=` : 幂赋值； `&=` : 按位与赋值； `|=` : 按位或赋值； `^=` : 按位异或赋值； `<<=` : 左移赋值； `>>=` : 右移赋值。

有些语言中，有种很常用的变量一元运算符，用于变量递增或递减，比如 `i++`。但 Python 不支持这样的运算符，只能使用 `i += 1` 实现同样的功能。注意：在 Python 中写 `++i` 并不会报错，但它仅仅表示 `+(+i)`（正的正数），并不会改变 i 的值。
