...
1. **元音字母个数**：编写一个函数，输入一个字符串，函数返回字符串中元音字母（a, e, i, o, u）的个数。

2. **列表中第二大的元素**：编写一个函数，查找给定列表中的第二大元素。

3. **字符串长度列表**：编写一个函数，将给定的字符串列表转换为每个字符串长度的列表。

4. **订制披萨**：编写一个函数，模拟披萨制作过程。要求把每一种配料作为一个函数参数传递给函数，函数打印出所有的配料。
