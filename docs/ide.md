...
有几个不同的选择，笔者建议初学者先使用第三方的免费在线开发环境入门，在对 Python 有了一定了解后，需要继续深入学习的时候再在自己的电脑上安装更为专业的开发环境。
...
如果读者还没有自己常用的在线 IDE，可以打开 Google 搜索“online IDE”，能够找到非常多的免费的在线编程网站，可以任意选择一个使用。出现在笔者搜索结果上的前两个结果分别是：
...
上面介绍的几个网站，写好 Python 程序都是在它们的服务器上运行的，它们会把运行结果显示在网页上。还有一类更简化的开发环境，它们会直接在用户的浏览器里运行 Python 程序。比如这是笔者调用 pyscript 搭建的页面 <https://qizhen.xyz/python>，也可以用于学习时随手测试。这类网页工作的原理是先把 Python 程序翻译成 JavaScript 代码，而浏览器可以直接运行 JavaScript 程序，这样就间接的运行了用户输入的 Python 程序。
...
对于初学，这个基本的安装包就足够了。但是如果将来会在多个项目中使用到 Python，可能还要考虑不同版本的不同版本的 Python 和扩展库等发生不兼容的问题。比如，某个项目可能必须运行在 Python 3.9 而另一个项目必须运行在 Python 3.12；或者某个项目必须使用 PyTorch 1.x （这是一个 Python 的用于机器学习的扩展库），而另一个项目必须使用 PyTorch 2.x 等等。自己管理多个版本的 Python 和各种库的组合是非常麻烦的。所以，笔者建议最好使用专业的工具来管理所需的库和环境。
...
笔者平时以上这些开发环境都会使用到。笔者在学习或者测试一些小程序比如面试题目的时候，一般使用第三方的在线编辑环境；在开发个人使用的应用程序或者网页时，会采用 PyCharm；在工作中，进行数据统计、机器学习等类型的项目时，采用公司搭建的类似 Jupyter Notebook 的环境进行开发；而开发工作相关的应用程序、网络应用等则使用 VS Code。
