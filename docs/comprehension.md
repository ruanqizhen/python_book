...
除了直接构建幂集的算法外，我们还可以借用二进制数据每个数位的数字作为掩码，来构造出一个集合的幂集。假设一个集合有 n 个元素，那么考虑数值 0 ~ $2^n$，用二进制来表示这些数，二进制数的每个数位表示原集合中的一个元素，0 表示该元素不存在； 1 表示该元素存在。那么，这 $2^n+1$ 个数，每个就恰好可以用来表示原集合的一个子集。所有的数构成了原集合的全部子集，其程序实现如下：
...
海象运算符很容易破坏程序的可读性，因为它经常要和其他的运算符一起使用，很容易搞不清楚不同运算符的优先级。尤其是它与赋值表达式长得太像了，很容易让人不自觉地将赋值语句的规则套用过来。为了提高程序可读性，可以在所有使用海象运算符的地方都用括号套起来，这样就容易区分计算的优先级了。下面看几个例子，大家可以先猜测一下程序输出是什么：
