...
列表推导式的优点是代码简介。但是过于复杂的列表推导式可能会比较难以读懂，代码写的太简介的确也增加理解难度，这就好比古文比白话文难懂一样。当推导式变得复杂时，可以考虑使用传统的循环结构。下面给出一些比较复杂的列表推导式，笔者不建议才产品中使用类似的复杂列表推导式，但作为学习，理解它们可以锻炼对于 Python 语言的熟练度。
