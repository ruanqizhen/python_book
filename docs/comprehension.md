...
在数学中，一个集合 `S` 的幂集（Power Set），记作 `P(S)`，是指由 `S` 的所有子集（包括全集 `S` 本身和空集 `{}`）构成的集合。例如，如果 `S = {1, 2}`，那么它的幂集就是 `P(S) = {{}, {1}, {2}, {1, 2}}`。如果一个有限集合 `S` 包含 `n` 个元素，那么它的幂集 `P(S)` 将包含 $2^n$ 个元素。

在 Python 中，集合（`set`）的元素必须是“可哈希”的（hashable），即不可变的数据类型。由于集合本身是可变的，所以一个集合不能作为另一个集合的元素。因此，我们通常用一个列表（List）来存储所有的子集，最终得到一个列表，其元素为集合，例如 `[set(), {1}, {2}, {1, 2}]`。

最直观、最容易理解的算法是：从一个只包含空集的列表开始，逐一遍历原集合中的每一个元素。每遇到一个新元素，就将它添加到当前已经存在的所有子集中，从而形成一批新的子集，再将这批新的子集加入到结果列表中：
...
除了上面提到的迭代方法，我们还可以利用二进制掩码法构造一个集合的幂集。假设，一个有 `n` 个元素的集合，其幂集有 $2^n$ 个子集。我们可以用从 0 到 $2^n - 1$ 的所有整数来表示这 $2^n$ 个子集。具体方法是，将这些整数表示为 `n` 位的二进制数。如果二进制数的第 `j` 位是 `1`，则表示原集合中（按某个固定顺序排列）的第 `j` 个元素被包含在这个子集中；如果为 `0`，则不包含。

比如： `s = {a, b, c}` (n=3)。我们需要 $2^3 = 8$ 个掩码，即 0 到 7。
...
赋值表达式（Assignment Expression）使用运算符 `:=` 来为变量赋值。它也被昵称为“海象运算符”，因为它看起来就像是海象的眼睛和大牙。与 Python 中的[赋值语句](variable#声明和赋值)（使用等号，比如 `a = 5`）相比，赋值语句是一个“[语句](hello_world#语句)”，它不能出现在需要[表达式](hello_world#表达式)的地方，比如在 if 或 while 语句的条件中，或者作为其他表达式的一部分。但赋值表达式，是一个表达式，它可以出现在任何使用表达式的地方。
