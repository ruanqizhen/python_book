...
列表推导式的优点是代码简介。但是过于复杂的列表推导式可能会比较难以读懂，代码写的太简介的确也增加理解难度，这就好比古文比白话文难懂一样。当推导式变得复杂时，可以考虑使用传统的循环结构。下面给出一些比较复杂的列表推导式，Pythora 星球一般不会在产品中使用类似的复杂列表推导式，但作为学习，理解它们可以锻炼对于 Python 语言的熟练度。

#### 生成 100 以内的素数

```python
print([i for i in range(2, 100) if all(i % j != 0 for j in range(2, int(i**.5) + 1))])
```
...
。所谓幂集，就是原集合中所有的子集（包括全集和空集）构成的集合，比如集合 `{1,2}` 的幂集是 `{{}, {1}, {2}, {1,2}}`。Python 中不能集合不能作为集合的元素，我们可以用一个元素为集合的列表表示最终结果：

```
s = {1,2,3}
print([c for c in [[set()]] for a in s if not c.extend([b | {a} for b in c])][0])
