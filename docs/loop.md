...
* 处理大量数据： 有一个包含数千或数百万条数据的列表，并且需要对每条数据进行处理，不可能手动为每条数据编写处理代码。使用循环，可以为所有数据编写一次处理逻辑。
* 重复执行： 例如，编写一个游戏，让玩家尝试多次，直到赢得比赛或放弃。可以使用一个循环来允许玩家多次尝试。
...
Python 中的迭代器（iterator）是一种数据对象，它们都有一个共同的方法，称为 `__next__`（或者通过内置函数 next() 调用）。调用此方法即可获取下一个数据。可以想象，如果有一组数据，如果使用迭代器，每次找到当前数据的下一个数据，这样持续下去，就可以遍历这组数据中的每一个数据。目前，我们只需要大致了解其概念，后续将专门开设一节，专门讲解[迭代器的工作原理](iterator)。
...
我们通常可以在循环中显示迭代对象的数据、将数据处理后保存、发送到其他设备，或生成一个新的列表等。但通常不建议在循环中修改、删除或增加被迭代对象，因为此类操作可能导致意外结果。循环会迭代修改前的数据还是修改后的数据呢？如果被迭代对象长度发生变化，那么下一个数据应该是谁呢？比如，读者可以猜测下面的程序运行结果是什么：
...
上面这类操作非常危险。因为 for 循环内部维护了一个指向当前元素的索引，当你在循环中插入数据时，列表变长了，原有的元素被推到了后面，导致循环可能重复访问同一个元素或永远无法到达列表末尾，从而引发[死循环](#死循环)。所以，最好不要在循环内增删被迭代对象，如果需要修改，可以使用[列表的切片](list#修改列表)等相应方法，或者[推导式](comprehension)、[高阶函数](high_order)等方法进行修改。

### range() 函数

range() 是 Python 的内置函数，返回一个 range 对象（不可变的整数序列，按需生成其中的整数）。它本身不是迭代器（没有 `__next__` 方法），但它是可迭代的，可以用 for 循环遍历。这个函数在循环中特别有用。
...
需要注意的是： range() 生成的序列不包括 stop 指定的值。

在 Python 2 中，range() 返回一个列表；而在 Python 3 中，它返回一个不可变的序列对象（range object）。虽然它表现得像迭代器一样只在需要时生成数值（从而节省内存），但它实际上比迭代器功能更丰富（支持索引访问和多次遍历）。因此，如果直接打印 range() 函数，是不会看到期望的数列的，需要将其转换为列表或在循环中迭代它。

使用 list() 函数把 range() 返回的不可变的序列对象转换成列表：
...
如果两个列表的长度不同，zip 会在最短的列表结束时停止（即截断）。若希望保留所有数据（以最长的列表为准），可以使用 itertools 模块中的 [zip_longest() 函数](iterator#打包拆包)。它会自动用 None（或指定的 fillvalue）来填充较短列表中缺失的对应元素。比如：
...
当 break 语句在循环中被执行，它会立刻终止所在的循环，跳出循环。程序会继续执行循环之后的代码。break 最常见的应用场景是：若循环过程中已满足预设条件，比如已经找到了要找的数据，那就不需要再执行后面的循环迭代了，直接跳出循环可以节省程序运行时间。
...
循环带着一个 else 子句是比较独特的，在其它编程语言中比较少见。而且，这个 else 比较容易引起歧义，直觉的理解可能是：如果条件不满足就进入 else 块，那就应该是循环出了意外则进入 else；但实际上恰恰相反，它真正的行为是当循环正常完成时，进入 else 子句。此外，当迭代次数为 0（即输入的可迭代对象为空）时，程序也会进入 else 子句。这的确有点令人迷惑，所以有人建议始终避免在循环语句后面添加 else 块，毕竟它不是必须的，其它语言都没有这个功能。
...
编写 while 循环时，需特别注意避免死循环。要始终检查循环条件和循环体，确保循环有明确的退出条件。 可以使用 break 语句在满足某些特定条件时跳出循环，避免死循环。
...
猴子发现一棵桃树，第一天吃了树上一半的桃子后仍不满足，又多吃了一个。第二天早上又将剩下的桃子吃掉一半，再加一个。以后每天早上都吃掉剩下的一半再加一个。到了第 n 天早上，发现树上只剩下 1 个桃子。问树上原本有多少个桃子？
