...
Python 中的迭代器（iterator）是一种数据对象，它们都有一个共同的方法，叫做“得到下一个”，调用这个方法，就可以得到下一个数据。可以想象，如果有一组数据，如果使用迭代器，每次找到当前数据的下一个数据，这样持续下去，就可以遍历这组数据中的每一个数据。目前，我们只需要了解这个大概，后面还会单开一节，详细讲解[迭代器的工作原理](iterator)。
