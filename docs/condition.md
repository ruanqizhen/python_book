...
笔者在最初在最初学习 Python 的时候，不是特别喜欢 Python 的这种设计。主要是觉得缩进是比较容易搞错的，很容易多几个、少几个空格。毕竟在其它语言中，缩进只影响美观不影响逻辑，因此它没那么重要，容易被忽视。不过，随着时间的推移，笔者发现自己的担心是有些多余的，在习惯了 Python 的代码分块方式之后，并没有发现它会导致更多的错误，而且它也确实可以提高代码的整洁度和可维护性。
...
如果索引是整数，或[枚举类型数据](oop2#枚举)，使用这种方式还可以代替简单的模式匹配语句，比如：
