...
**Python 3.15 变化**：从 3.15 起，`open()` 不写 `encoding` 参数时，在所有平台默认使用 UTF-8，不再跟随系统 locale（以前在中文 Windows 上默认是 GBK，省略参数容易产生乱码）。“尽量显式指定编码”依然是好习惯，但理由变了：现在是为了表达意图；如果确实想要以前那种跟随系统的行为，可以写 `encoding="locale"`。
