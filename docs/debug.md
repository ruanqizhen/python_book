...
另外，Python 3.15 的报错信息更智能了，能覆盖更多情形（例如删除一个拼写相近的属性时，会提示 Did you mean 并给出正确的属性名）。
...
print 和日志回答的是“程序哪里做错了”，但程序太慢时需要回答的是“时间花在了哪里”。Python 3.15 把性能分析工具收拢到 `profiling` 包下：`profiling.tracing`（从 `cProfile` 搬迁而来，原名仍可作别名使用）负责精确计时；新增的 `profiling.sampling`（代号 Tachyon，采样率最高可达 1MHz）可以 attach 到正在运行的进程并生成火焰图。旧的 `profile` 模块已弃用（3.17 移除），不要再用。
