...
在设计函数的时候，我们经常会面临是否需要抛出异常的选择。比如，函数的输入数据应该是一个非空的列表，可以用户输入了一个空列表，甚至是其它类型的数据，那么我们是应该让函数抛出一个异常，还是应该让函数返回一个特殊值，比如返回 None 或空列表等。

这其实并没有一个固定答案，要根据具体的情况来判断，而且有很多因素可能会影响我们的决定。

### 行为保持一致

我们设计一个函数的行为，还要参考同一项目中其它函数。同一个项目的不同部分，最好可以保持一致的行为。比如，项目中大多数的函数都是使用异常来处理错误的，那么新设计的函数也应该利用异常处理机制。反之，如果项目中大多数函数是利用返回值作为错误处理机制的（有错误就返回 None，否则返回一个有意义的数据），那么新设计的函数也应该才去类似机制。
...
异常处理机制有很明显的优点，它有清晰的处理逻辑，可以携带纤细的错误信息，可以支持复杂的处理流程。但他也有缺点，最主要的缺点是有额外的性能开销，毕竟它要携带更多的数据，和控制更复杂的跳转逻辑。从代码的可读性考虑，异常处理机制更清晰明了；但是追求极致性能的程序可能更适合使用返回值作为错误处理机制。
