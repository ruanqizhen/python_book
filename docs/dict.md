...
### 不可变字典（frozendict，Python 3.15+）

Python 3.15 新增了内置的 `frozendict`：创建后不可修改的映射。当所有键和值都可哈希时，`frozendict` 本身也可哈希，因此能作为字典的键或放入集合——这是普通 `dict` 做不到的。注意它不是 `dict` 的子类。

```python
config = frozendict(host="localhost", port=8080)
print(config)  # 输出: frozendict({'host': 'localhost', 'port': 8080})

# config["port"] = 9090  # 报错：TypeError，不支持修改

# 键和值都可哈希时，可整体作为另一个字典的键
cache = {frozendict(a=1): "hit"}
print(hash(frozendict(y=2, x=1)) == hash(frozendict(x=1, y=2)))  # 输出: True
```

典型用途是存放不允许被意外修改的默认配置，或把一组键值整体当作键使用。它与 `dict` 的关系，正如 `frozenset` 与 `set` 的关系：可变版本用于日常读写，不可变版本用于需要只读保证或哈希的场合。
...
在 Python 编程中，列表（List）、元组（Tuple）、字典（Dict）和集合（Set）是四个最核心、最常用的内置数据容器。初学者常常混淆它们的使用场景，因此我们在此进行一次多维度的横向对比，以便在实际编程中做出最恰当的选择。Python 3.15 新增了不可变字典（FrozenDict），它是字典的只读对应物（正如冻结集合之于集合），下表也一并列出。

### 📌 四大容器特性对比表

| 容器类型 | 声明语法 | 是否有序 | 是否可变 | 元素重复性 | 查找/检索时间复杂度 | 典型应用场景 |
| :--- | :--- | :---: | :---: | :---: | :---: | :--- |
| **列表 (List)** | `[a, b, c]` | **是** | **是** | 允许重复 | $O(n)$ | 顺序存储数据、频繁增删、动态队列 |
| **元组 (Tuple)** | `(a, b, c)` | **是** | **否** | 允许重复 | $O(n)$ | 结构化记录、函数多返回值、字典的键 |
| **字典 (Dict)** | `{k: v}` | **是** *(3.7+)* | **是** | 键唯一，值可重复 | **$O(1)$** | 快速键值对检索、JSON 数据解析 |
| **集合 (Set)** | `{a, b, c}` | **否** | **是** | 元素唯一 | **$O(1)$** | 快速去重、数学集合运算（交、并、差） |
| **不可变字典 (FrozenDict)** *(3.15+)* | `frozendict({k: v})` | **是** | **否** | 键唯一，值可重复 | **$O(1)$** | 只读配置、可哈希时可作字典的键 |

### 🔍 核心要点解析

*   **有序性（Ordering）**：
    *   **列表**和**元组**是天生有序的，可以通过索引（如 `arr[0]`）进行精确访问。
    *   **集合**是完全无序的，不能通过索引访问。
    *   **字典**从 Python 3.7 开始，其底层实现保证了遍历顺序与插入顺序一致，但它本质上还是通过“键”（Key）而非整数索引来访问数据的。
*   **可变性（Mutability）**：
    *   **列表**、**字典**和**集合**是可变的，可以随时添加、修改或删除其中的元素。
    *   **元组**是一旦创建就绝对不可变的。不可变性让元组更安全、访问速度更快，并且可以作为字典的键或集合的元素（因为它是“可哈希的”）。
*   **查询性能（Search Complexity）**：
    *   在**列表**或**元组**中查找某个元素（如 `x in arr`），需要从头到尾进行遍历，时间复杂度是线性的 **$O(n)$**。如果数据量达到百万级别，速度会显著变慢。
    *   **字典**和**集合**底层基于**哈希表（Hash Table）**实现。在其中查找键或元素（如 `key in dict` 或 `x in set`），只需进行一次哈希计算即可定位，时间复杂度是常数级别的 **$O(1)$**，其查询速度几乎不受数据量大小的影响。因此，对于需要频繁查找的场景，应当优先选择字典或集合。
