...
* 序列类型:
   ...
   * [元组（tuple）](list#元组)： 有序的不可变对象集合，例如：`(1, 2, 3, 'a', 'b')`。
...
* [映射类型](dict):
   ...
   * 不可变字典（frozendict，Python 3.15+）： 与字典类似，但是它是不可变的；当键和值都可哈希时，它本身也可哈希，能作为字典的键。
   
* [None 类型](calculation#变量指针的比较):
   * NoneType：只有一个值 None。用于表示一个值的缺失或空。
   
* 二进制类型:
   * [字节序列（bytes）](string#字节序列)： 包含字节的不可变序列，例如：b'hello'。
...
* 其他数据类型:
   许多 Python 模块和库都对数据类型进行了扩展。在使用了这些模块的程序中，很可能会遇到其他数据类型，如 `numpy` 库中定义的 `ndarray` 或 `pandas` 库中定义的 `DataFrame` 等。
...
变量的名字是一种[标识符](hello_world#标识符)，遵循标识符的定义规范。它由字母、数字和下划线 `_` 组成，其中字母不限于英文字母，也可以是中文等其他语言的字符；变量名不能以数字开头。比如，`name_1` 是一个合法的变量名，而 `1_name` 不是。
...
如果给单个变量赋值多个数据，Python 会自动将所有数据打包成一个[元组](list#元组)：
...
链式赋值一般用于[不可变数据类型（下文会详细解释）](#数据的可变性)。链式赋值会导致所有变量都指向同一个数据，如果该数据是可变的，一旦其中一个变量修改了数据，其他所有指向该数据的变量的值也会随之改变，这可能会引发一些意想不到的问题。
