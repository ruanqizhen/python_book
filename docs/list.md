...
Python 的列表（list）是一种有序的集合，是 Python 中最基本的数据结构之一。我们可以使用方括号 [] 创建一个列表，这是最基础的方法，通过简单地在方括号中放置元素，元素之间用逗号分隔，比如
...
我们还会经常使用 list() 函数，将其它[可迭代对象](loop#可迭代对象和迭代器)转换为列表，比如：
...
解包（Unpacking），也称拆包。它是 Python 中的一种方便的功能，它允许你将列表中的元素“解包”（即分解）到变量中。这意味着你可以在单个操作中将列表中的每个元素赋给多个变量。这通常用于函数返回多个值时，或者在循环、赋值操作中。
