...
除了上面这几种方法，后面还会介绍到一些同样可以删除列表元素，比如[列表推导式](generator#列表推导式)、[filter() 函数](high_order#filter)等。我们只是以列表删除元素为例，实际上，对于大多数任务来说，Python 都有多种解决方案可供选择。本书后续的内容中，会经常发现，在不同章节中使用各自方法完成了一些极其类似的功能。Python 就是这样灵活的语言，缺点是它需要我们学会权衡利弊来选择最适合当前问题的解决方案。
