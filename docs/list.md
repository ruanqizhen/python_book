...
解析：从索引 -2（即数字 8）开始，向左每隔 3 个数取一个，直到索引 2（即数字 2，但不包含该位置）。因此取到了 8，然后跳过 7、6 取到 5，再跳过 4、3 到达 2（停止）。
...
需要注意的是，切片赋值的本质是“替换”：它会先将切片选中的元素移除，然后在该位置填入新的元素序列。填入的元素数量不需要与移除的数量一致。

比如 `fruits[1:1]`，这表示从索引 1 开始，到索引 1 结束（不包含索引 1），这实际上选不中任何元素，是一个空切片（可以想象为索引 0 和 索引 1 之间的缝隙）。将这个缝隙“替换”为新列表，效果就等同于插入。
...
当我们改变 `board_list[0][0]` 的值的时候，其它几个行列表中的值也被改变了。因为这些复制产生的列表实际上都是同一个列表（指向了同一块内存地址）。

若需生成深拷贝数据（即每一行数据相互独立），可以使用[列表推导式](comprehension)初始化多维列表。比如，我们需要生成一个 8*8 的棋盘，棋盘每个格内的初始数据都是 0，那么需要这样写：
...
上面的程序是一个链式比较操作，展开逻辑：`A op1 B op2 C` 等价于 `(A op1 B) and (B op2 C)`，因此结果为 True，但不熟悉的读者可能会考虑，无论是 `==` 优先级高，还是 `in` 优先级高，结果都应该是 False 嘛。
...
* insert() - 在指定索引处插入一个元素。
* remove() - 删除指定的元素。
* pop() - 删除指定索引处的元素并返回它。如果不指定索引，默认删除并返回列表的最后一个元素。

remove() 与 pop() 的主要区别在于：remove() 是根据值来删除（不知道索引在哪），而 pop() 是根据索引来删除（通常用于利用索引位置或处理末尾元素）。
...
需要注意的是，copy() 方法也只是浅拷贝。所谓浅拷贝的意思是：copy() 方法虽然会生成新的列表，但它不会复制列表内的元素。对于嵌套列表或包含其他可变数据的列表，新复制的列表中的元素依然还是指向原来列表中的元素的。比如：
...
从外观上看，元组与列表的唯一区别在于，元组使用小括号表示，列表使用方括号表示。这只是表面的，本质上最主要的区别在于，列表是可变的，我们可以修改、添加或删除列表中的元素；而元组是不可变的，一旦创建，就不能再修改、添加或删除元组中的任何元素了。由于元组不可变，它的运算和功能会少一些，比如元组不支持 append() 这类用来改变元素的方法，只支持读取数据的方法。由于，元组不可变，它更加安全，更加节省内存，访问速度也更快。当需要创建一个不变的列表时，我们应该使用元组。

Python 会将使用逗号分隔的多个数据自动组成元组。：
...
一个常见的错误是在使用数据时，后面多加了一个逗号，导致 Python 将其自动组成元组而非单一值：
...
1. 反转列表：编写一个程序，将列表中的数据反向排列。
2. 按奇偶拆分列表：把一个整数列表，按照其元素的奇偶性，拆分成两个列表分别保存奇数和偶数。
3. 次大数据：在一个实数列表中，找出第二大的数。
4. 列表分块：输入一个列表和一个整数 n，将列表按照每 n 个元素为一个子列表进行切割。例如列表 [1,2,3,4,5]，n=2，应输出 [[1,2], [3,4], [5]]。
