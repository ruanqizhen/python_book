...
时间复杂度：每次旋转操作需要 $O(n)$ 时间，我们旋转 k 次，因此总的时间复杂度为 $O(nk)$。但在最坏的情况下，例如 k 接近 n/2，时间复杂度为 $O(n^2)$。 空间复杂度：我们只用了一个额外的变量 previous，因此空间复杂度为 $O(1)$。
...
注意：上面的 Python 实现为了代码简洁，使用了额外的列表空间，并非原地排序。在对空间敏感的场景下，标准的快速排序应通过索引交换在原数组上进行。
...
计数排序的时间复杂度是 $O(n)$，比快速排序的复杂度又降低了一级。从上图的程序中也可以看出来，它没有嵌套的循环。说明计算次数与输入数据长度是呈线性关系的。计数排序虽然快，但适用范围非常有限。对于快速排序来说，他能够给任何可以比较的对象排序，但是计数排序只适用于非负整数，并且也只是对于小范围的整数，可以非常快速。因此计数排序的应用远没有快速排序那么普及。


## sorted（） 高阶函数

Python 内置的 sorted() 函数采用了一种名为 Timsort 的算法。它是一种结合了归并排序和插入排序的混合稳定排序算法，特别针对现实世界中部分有序的数据进行了优化：对于较小的数据块或近似排序的数据块，sorted（） 采用插入排序；对于较大的输入，sorted（） 先把它们分成小块，每个小块还是采用插入排序，然后使用归并排序合并小块的排序结果。
