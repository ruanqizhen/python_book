...
在介绍循环的时候，曾经提到[迭代器](loop#可迭代对象和迭代器)。迭代器允许程序员遍历列表、字典等数据容器中的所有元素。迭代器有两种主要实现方法，一种依赖面向对象的编程方式，将在[使用面向对象的编程方式实现迭代器](iterator)一节详细介绍；另一种是生成器（Generator），将在本节重点讲解。生成器是一种高效的创建迭代器的方式，而且它允许我们按需生成数据，而不是一次性生成并存储所有数据。创建生成器有两种主要的方法：生成器函数和生成器表达式。
...
需要注意的是，一个函数如果同时包含 yield 和 return，那么 return 的行为和普通函数是不一样的。当生成器函数遇到 return 时，生成器会抛出 StopIteration 异常，并且 return 的值会作为 StopIteration 异常的值。
...
# 第三次调用 next()，会触发 StopIteration 异常，并返回 return 的值
try:
    print(next(gen))
except StopIteration as e:
    print(f"Caught StopIteration: {e.value}")  # 输出: Caught StopIteration: Return value
```
...
在实际应用中，直接调用 next() 函数的情况较少，通常会通过循环或推导式来迭代生成器对象。比如下面的示例，它是一个[斐波那契数列](recursive#计算斐波纳契数列)的生成器，在循环中迭代这个生成器对象，每次会得到一个斐波那契数：
...
上面两段程序的区别在于效率。假设我们需要很大量的数据，比如生成一千万个斐波那契数。直接返回列表的方式，要等全部数据都生成好之后，才能返回，返回时间会延长。而且这些数据要一次性装入内存，内存占用过大。如果要求更大量的数据，可能内存都无法容纳。而对于生成器来说，它每产生一个数据就会返回一个数据，处理这个数据的代码段无需等待，就可以立刻运行处理当前的数据了。而且内存中每次只需加载正在处理的数据，无需加载数列中的其他数据，从而显著降低内存占用。
...
我们可以使用 yield from 语句，在生成器函数中，直接输出另一个生成器产生的值。换句话说，yield from 语句可以将其他生成器或可迭代对象产生的值当作自身生成器的输出。比如，我们已经有了两个简单的生成器： generator1 和 generator2。现在又需要创建一个新的生成器，它首先迭代 generator1 生成的所有的值，输出，然后再迭代 generator2 生成的所有的值，输出。那么，在新的迭代器中，可以不必迭代已有的生成器，而是使用 yield from 直接使用它们的结果：
...
**Python 3.15 补充**：生成器表达式也支持 `*` 解包，`list((*L for L in lists))` 同样可以平展嵌套列表（与推导式解包 PEP 798 一致）。不过 `yield from` 依然是更清晰的写法，优先使用它。
...
生成器和生成器表达式是惰性生成结果的，这表示数据是在实际需要时才生成的，而不是一开始就生成所有可能的数据。这种方法可以显著减少内存使用，特别是在处理大型数据集时。比如在下面这个示例中：
...
count_up 是一个生成器函数，它有一个无限循环。但是程序并不会运行无限次，因为，只有在程序中调用 next() 时，它才会执行一次。程序中的 for 循环运行 10 次，每次调用 next(it)，这个 for 循环实际上限制了生成器生成值的次数，只生成 10 个值。

惰性生成的优点是提升了内存效率，但是它也有一些潜在的风险，需要特别注意：

#### 限制次数

调用生成器要注意限制次数，否则它们运行无数次可能还是会造成内存不足等问题。有些操作可能会在无意间，忘记了限制调用次数。比如把生成器作为参数传递给支持不定数量参数的函数：
...
在上面的程序中，`print(*count_up())` 试图将一个无限的生成器解包成参数列表。Python 会尝试一直运行生成器直到它耗尽（但它永远不会耗尽），并试图在内存中构建一个无限大的参数元组。这会导致程序立即卡死，并最终导致内存溢出崩溃。切勿对无限生成器使用 list() 转换或 * 解包。
...
因为数据产生有一个过程，如果在这中间，数据源发生变化了，可能会导致产生的数据也发生改变，甚至产生一些意料之外的结果。请读者预测一下，下面这个排列组合生成器产生的结果是什么？
...
在 Pythora 星球，有一个选择题万能公式：三长一短选最短；三短一长选最长；差别不大都选 C。


这是一个展示 Python 生成器“绑定时机”特性的经典陷阱：
- 外层循环立即绑定：生成器表达式在定义时，最外层的 for 循环（for i in x）会立即执行对 x 的查找和绑定。因此，i 的迭代范围被锁定为定义时的 x，即 `[1, 2]`。
- 内层循环惰性查找：内部的循环（for j in x）和具体的表达式计算是惰性的。它们只有在生成器真正运行（被迭代）时才会被执行。当程序运行到 `print(*comb)` 时，生成器才开始工作，此时内部循环去查找变量 x，读取到的是最新的值 `[3, 4]`。

因此，i 遍历 `[1, 2]`，而 j 遍历 `[3, 4]`，组合结果为：(1, 3), (1, 4), (2, 3), (2, 4)。
