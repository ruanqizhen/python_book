...
前面几节在讨论面向对象编程的时候，主要是从实现的角度来考虑的：假设我们已经知道需要哪些类，应该如何编写代码。接下来两节，我们换一个角度，来考虑一下如何设计面向对象的程序：对于一个问题，应该构建哪些类，它们之间关系如何。

在前文中，我们已经详细介绍了面向对象编程的三大特性：[封装](class#封装)、[继承](class#继承)、[多态](class#多态)。在设计程序时，这三大特性亦然是最重要的考虑因素，也就是说，在设计类时，我们需要考虑类中有哪些属性和方法，是否可以继承其它一些类中的属性和方法。除此之外，还有一些比较常用的设计方式和技巧可以考虑：
