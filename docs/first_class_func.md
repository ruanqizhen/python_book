...
这样实现的代码允许用户在不改变 operate 函数的内部逻辑的情况下，使用不同的操作或策略。这样可以避免重复的代码，因为所有的操作都使用了同一个 operate 函数。同时使代码更加清晰和可读，特别是当操作变得更加复杂时。
...
有时，我们可能希望根据某些条件动态创建并返回不同的函数。例如：
...
上面的代码中，使用循环创建了三个内部函数，创建这三个内部函数的时候，i 的值分别是 0, 1, 2。因此，当 x= 10 的时候，我们期望这三个内部函数分别返回 10, 11, 12，也就是 x+i 的值。然而，执行这段程序，三个 print 语句的输出都是 12。

之所以输出都是 12，是因为 Python 的闭包是后期绑定（Late Binding）的。这意味着内部函数 func 在被定义时，并没有把 i 的当前值（0, 1, 2）复制进去，而是记住了 i 这个变量的引用。当循环结束时，变量 i 的值最终停留在了 2。等到我们后来调用 `functions[0](10)` 时，程序才去查找 i 的值，此时它读到的 i 已经是 2 了。因此，所有函数都使用了同一个最终的 i 值。

要解决这个问题，我们需要在函数定义时就将 i 的值固定下来。最常用的方法是使用函数的默认参数，因为默认参数的值是在函数定义时计算的：
...
闭包可以用于读数据和函数的[封装](class#封装)，因为，闭包可以用来隐藏数据，把外部函数中的局部变量隐藏起来。外部程序需要访问数据，只能通过定义好的函数进行访问。
...
柯里化的一个经典例子是对于一个二元函数 f(x, y)，柯里化后，得到一个函数 g(x)，其返回值是另一个函数，这个返回的函数用于处理 y。比如，下面示例是一个简单的加法函数，它需要两个输入参数，但是我们可以把它做成一系列单参数函数：
...
另一种应用场景是：需要调用一个库函数 foo，该函数要求传入一个单参数函数作为其参数 bar。但我们已有的函数通常含多个参数。此时，可以使用偏函数将多参数函数的其他参数固定，包装成一个单参数函数，从而满足参数 bar 的要求。
...
在上面的示例中，我们将定义好的 lambda 函数分配给了变量 f，后续的程序可以使用变量 f 来调用这个 lambda 函数。f 作为指向一个函数的变量，它的功能与直接定义一个名为 f 的函数基本相同，比如：
...
在 Python 中，`def f(x, y):` 这种写法本质上也是将一个函数对象赋值给了变量名 f。使用 lambda 和使用 def 的主要区别在于：
- 代码风格：lambda 适合编写那些“用完即扔”、逻辑简单的函数，不需要专门起个名字占位。
- 元数据：使用 def 定义的函数，其 [`__name__` 属性](module#导入时运行)是函数名本身，在调试和报错时更清晰；而 lambda 函数的 `__name__` 属性是 `<lambda>`，调试时不如普通函数直观。

在某些编程语言中（例如 JavaScript），程序员倾向于使用匿名函数实现所有功能，并通过变量而非函数名来引用它们。
...
嵌套定义的 lambda 函数与嵌套定义的普通函数一样支持闭包，也就是内部的 lambad 函数可以访问并操作外层函数的变量与参数。
...
1. 编写一个函数 `apply_func(func, x, y)`，接受一个函数 func 和两个数 x、y，接受一个函数 func 和两个参数 x、y，返回 `func(x, y)` 的结果。例如，`apply_func(lambda a, b: a + b, 3, 5)` 应返回 8。
