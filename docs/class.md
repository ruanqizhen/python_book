...
初始化方法（`__init__`）用于初始化对象的状态。虽然在很多其他语言中（如 Java/C++）类似的函数被称为“构造函数”，但在 Python 中，严格来说 `__new__` 才是负责创建实例的构造方法，而 `__init__` 只是负责在实例创建后对其进行配置。不过，在日常交流中，人们常习惯将 `__init__` 称为构造方法。[后文将会详细介绍 `__new__` 方法](objects#__new__-方法)，这里主要介绍 `__init__` 方法。
