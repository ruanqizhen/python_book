...
在 Python 中，方法是绑定到对象或类的可调用对象，可以通过属性访问的方式调用。关于这一点，我们将在[对象的动态访问属性](objects#动态访问属性)中进行解释。

### 初始化方法

改为“初始化方法（`__init__`）用于初始化对象的状态。虽然在很多其他语言中（如 Java/C++）类似的函数被称为“构造函数”，但在 Python 中，严格来说 `__new__` 才是负责创建实例的构造方法，而 `__init__` 只是负责在实例创建后对其进行配置。不过，在日常交流中，人们常习惯将 `__init__` 称为构造方法。[后文将会详细介绍 `__new__` 方法](objects#__new__-方法)，这里主要介绍 `__init__` 方法。
...
调用类属性时，建议通过类名访问，以避免意外修改类属性或与对象属性混淆。尤其是有些时候，通过对象来调用，是不那么容易分清楚到底调用的是哪个属性。比如下面的示例：
...
Python 之所以可以采用鸭子类型，是因为它是动态类型语言，运行时不强制检查变量或参数的类型，因此我们可以把任何类型的对象传递给一个函数。而在 Java 这类静态类型检查的语言中，是不可能把任何对象都传递给一个函数的，它必须限定输入的对象只能是某各个类极其子孙类的对象。
...
这个示例，与前面一个非常类似，只是把 speak 方法变成了类方法。
...
双下划线（名称改写） 变量名以双下划线开头（如 `__private_variable`），Python 会进行“名称改写”（Name Mangling），将其改名为 `_类名__变量名`。 这里的初衷并非为了真正的“私有化”（因为你依然可以通过改写后的名字访问它），而是为了防止在类继承时，子类意外覆盖了父类的同名内部变量。但这样却是可以使其变得不容易从外部访问。比如：
